...
// Selectors
import {
  handleSaveProfileSelector,
  handleFetchProfileSelector,
} from './selectors';
...
export function* handleFetchProfile(action) {
  ...
  const { authenticationUsername, userAccount } = yield select(
    handleFetchProfileSelector,
  );
...
export function* handleSaveProfile(action) {
  try {
    const { username, drafts, preferences } = yield select(
      handleSaveProfileSelector,
    );
...
export function* handleSaveProfilePhoto(action) {
  ...
  try {
    ...
    const photoResult = yield call(
      ProfileApiService.postProfilePhoto,
      username,
      formData,
    );
